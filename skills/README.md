...
| [`agent-platform-implementation`](agent-platform-implementation/SKILL.md) | **Building and verifying.** The deployment runs and is quietly wrong: what actually crosses an agent-to-agent hop, why a caller invents data a sub-agent holds, guards a model cannot game, and the verification traps that report a score unrelated to your agent. |
