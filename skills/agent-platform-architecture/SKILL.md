...
| F14 | A Cloud Run agent identity receives **no default roles**, while an Agent Runtime one is created with `roles/aiplatform.agentDefaultAccess` bound to a project-wide `principalSet` — model prediction, `serviceusage.services.use`, logging, monitoring, trace and the API registry, and **nothing outside Agent Platform** | ⇒ on Cloud Run every permission the agent had as a service account must be re-granted; on Agent Runtime a bare identity already runs, which hides the difference until you move host. Either way Secret Manager, Storage and BigQuery need grants written by hand |
